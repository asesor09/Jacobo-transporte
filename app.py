...
def conectar_db():
    ...
    cur = conn.cursor()
    cur.execute("SET search_path TO public") # GPS para la DB
    return conn
...
if menu == "📊 Dashboard":
    ...
    if not df_v_all.empty or not df_g_all.empty:
        ...
        st.info(f"### 🚀 Utilidad Neta Actual: $ {res_mes['Utilidad'].sum():,.0f}")
        if res_mes['Utilidad'].sum() >= target: st.balloons()
    else: st.info("No hay datos mensuales.")
...
# --- 📑 HOJA DE VIDA (AQUÍ ESTÁ LA LÓGICA QUE NO VEÍAS) ---
