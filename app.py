...
# --- 3. MENÚ PRINCIPAL ---
# Definimos las opciones en una lista para asegurar que el nombre sea EXACTO
OPCIONES_MENU = ["📊 Dashboard", "🚐 Flota", "💸 Gastos", "💰 Ventas", "📑 Hoja de Vida", "⚙️ Tarifas", "⚙️ Usuarios"]

st.sidebar.write(f"👋 **{st.session_state.u_name}**")
st.sidebar.divider()
target = st.sidebar.number_input("🎯 Meta Utilidad ($)", value=5000000, step=500000)
menu = st.sidebar.selectbox("📂 SELECCIONE UN MÓDULO", OPCIONES_MENU)
...
if menu == "📊 Dashboard":
    ...
    m1.metric("📦 Vehículos Activos", v_act)
    m2.metric("📉 Gastos", f"$ {df_g['monto'].sum():,.0f}")
    m3.metric("📈 Ingresos", f"$ {df_v['monto'].sum():,.0f}")

    if not df_v.empty or not df_g.empty:
        df_v['fecha'] = pd.to_datetime(df_v['fecha'])
        df_g['fecha'] = pd.to_datetime(df_g['fecha'])
        res = pd.merge(df_v.groupby(df_v['fecha'].dt.to_period('M'))['monto'].sum(), 
                       df_g.groupby(df_g['fecha'].dt.to_period('M'))['monto'].sum(), 
                       on='fecha', how='outer', suffixes=('_v', '_g')).fillna(0)
        res['Utilidad'] = res['monto_v'] - res['monto_g']
        st.table(res.rename(columns={'monto_v':'Ingresos','monto_g':'Gastos'}).style.format("$ {:,.0f}"))
        st.info(f"### 🚀 Utilidad Neta Actual: $ {res['Utilidad'].sum():,.0f}")
    ...
    with st.form("f_g"):
        ...
        monto = st.number_input("Valor", min_value=0); fec = st.date_input("Fecha"); det = st.text_input("Nota")
...
# --- 📑 MÓDULO HOJA DE VIDA (BLINDADO) ---
elif menu == "📑 Hoja de Vida":
    st.title("📑 Documentación de Flota")
    v_data = pd.read_sql("SELECT id, placa FROM vehiculos", conn)
    if not v_data.empty:
        with st.expander("📅 Actualizar Vencimientos"):
            with st.form("f_hv"):
                ...
                pc_v = c2.date_input("Pol. Contractual"); pe_v = c2.date_input("Pol. Extra"); ptr_v = c2.date_input("Todo Riesgo"); to_v = st.date_input("Tarjeta Operaciones")
    ...
    else: st.warning("Primero registre vehículos en el módulo Flota.")
