...
def inicializar_db():
    ...
    # NUEVA TABLA DE CONFIGURACIÓN (Evita el UndefinedTable error)
...
def enviar_alertas_sistema(mensaje):
    try:
        ...
        if not conf or not conf[1]:
            st.error("⚠️ Configure primero las credenciales en el módulo 'Config. Alertas'.")
            return
...
# --- OTROS MÓDULOS ---
    ...
    st.title("🚐 Flota")
    # ... (Tu código de Flota igual al compartido)
    ...
    st.title("⚙️ Usuarios")
    # ... (Tu código de Usuarios igual al compartido)
