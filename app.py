...
# --- MÓDULO: DASHBOARD (Con Consolidado de Gastos añadido) ---
if menu == "📊 Dashboard":
    ...
    if len(rango) == 2:
        ...
        st.plotly_chart(px.bar(balance_df, x='placa', y=['Venta', 'Gasto'], barmode='group', color_discrete_map={'Venta': '#2ecc71', 'Gasto': '#e74c3c'}), use_container_width=True)

        # --- SECCIÓN NUEVA: CONSOLIDADO DE GASTOS POR CONCEPTO ---
        st.divider()
        ...
        df_concepto.columns = ['Concepto de Gasto', 'Total Gastado ($)']
        
        with col_con1:
            st.write("**Resumen Numérico:**")
            st.dataframe(df_concepto.style.format({'Total Gastado ($)': '${:,.0f}'}), use_container_width=True, hide_index=True)
        
        with col_con2:
            st.write("**Distribución Porcentual:**")
            fig_pie = px.pie(df_concepto, values='Total Gastado ($)', names='Concepto de Gasto', hole=0.4, color_discrete_sequence=px.colors.qualitative.Pastel)
            st.plotly_chart(fig_pie, use_container_width=True)

        st.download_button("📥 Descargar Reporte Completo (Excel)", data=to_excel(balance_df, df_g, df_v), file_name="Reporte_CE.xlsx")

# --- MÓDULO: GASTOS (CON EDICIÓN TOTAL + ELIMINACIÓN) ---
elif menu == "💸 Gastos":
    ...
    with tab1:
        with st.form("form_g"):
            ...
            monto, fecha, det = st.number_input("Valor ($)", min_value=0), st.date_input("Fecha"), st.text_input("Nota/Detalle")
    ...
    with tab2:
        ...
        ed_g = st.data_editor(df_g_edit, column_config={"id": None, "placa": st.column_config.SelectboxColumn("Placa", options=v_data['placa'].tolist())}, hide_index=True, use_container_width=True, num_rows="dynamic")
...
# --- MÓDULO: VENTAS (CON EDICIÓN TOTAL + ELIMINACIÓN) ---
elif menu == "💰 Ventas":
    st.title("💰 Control de Ingresos")
    v_data = pd.read_sql("SELECT id, placa FROM vehiculos", conn)
    tab1, tab2 = st.tabs(["💰 Nueva Venta", "✏️ Gestionar Ventas"])
    with tab1:
        with st.form("form_v"):
            v_sel = st.selectbox("Vehículo", v_data['placa'])
            cli, val, fec, dsc = st.text_input("Cliente"), st.number_input("Valor", min_value=0), st.date_input("Fecha"), st.text_input("Nota")
    ...
    with tab2:
        ...
        ed_v = st.data_editor(df_v_edit, column_config={"id": None, "placa": st.column_config.SelectboxColumn("Placa", options=v_data['placa'].tolist())}, hide_index=True, use_container_width=True, num_rows="dynamic")
...
# --- MÓDULO: HOJA DE VIDA (RESTANTE IGUAL) ---
    ...
    with st.expander("📅 Actualizar Fechas"):
        with st.form("form_hv"):
            ...
            pc_v, pe_v, ptr_v, to_v = c2.date_input("Contractual"), c2.date_input("Extra"), c2.date_input("Todo Riesgo"), st.date_input("Tarjeta de Operaciones")
    ...
    for _, row in df_hv.iterrows():
        ...
        for i, (name, fecha) in enumerate(docs):
            if fecha:
                ...
                elif dias <= 15: cols[i%4].warning(f"⚠️ {name} ({dias} d)")
