...
# --- INTENTO DE IMPORTACIÓN DE TWILIO (Evita que la app se rompa) ---
try:
...
def enviar_alertas_sistema(mensaje):
    ...
    # 2. Enviar WhatsApp (Solo si la librería está instalada)
    if TWILIO_INSTALADO:
        try:
        ...
        st.warning("No se pudo enviar WhatsApp porque la librería 'twilio' no está instalada.")
...
if menu == "📊 Dashboard":
    ...
    if len(rango) == 2:
        ...
        st.subheader("📊 Consolidado de Gastos")
        ...
        with st.expander("🔍 Detalle Movimientos"):
...
# --- MÓDULO: HOJA DE VIDA ---
    ...
    # Botón de alertas activo solo si Twilio cargó bien
    if st.button("🔔 Enviar Reporte a WhatsApp/Correo"):
        ...
        for _, r in df_al.iterrows():
            for doc, fec in [("SOAT", r['soat_vence']), ("TECNO", r['tecno_vence']), ("PREV", r['prev_vence'])]:
                if fec and (fec - hoy).days <= 15:
                    msg += f"- {r['placa']}: {doc} vence {fec}\n"; alertas = True
        if alertas: enviar_alertas_sistema(msg); st.success("Reporte enviado")
