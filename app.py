...
# --- 2. LÓGICA DE NOTIFICACIONES (CON LIMPIEZA DE CREDENCIALES) ---
def enviar_alertas_sistema(mensaje):
    try:
        ...
        # Limpieza de seguridad para evitar el error 535
        ...
        st.success(f"✅ Reporte enviado a {destino}")
    except Exception as e: 
        st.error(f"❌ Error de Autenticación: {e}")
        st.info("Revisa que el correo remitente sea el mismo que generó la clave de 16 letras.")
...
# --- MÓDULOS GASTOS Y VENTAS (RESTAURADOS) ---
elif menu == "💸 Gastos":
    ...
    with tab2:
        ...
        st.data_editor(df_ge, column_config={"id": None}, hide_index=True, use_container_width=True)
    ...
    st.data_editor(df_ve, column_config={"id": None}, hide_index=True, use_container_width=True)

# --- HOJA DE VIDA (RESTAURADO) ---
    ...
    with st.expander("📅 Actualizar Fechas"):
        with st.form("fhv"):
            v_sel = st.selectbox("Vehículo", v_query['placa']); v_id = v_query[v_query['placa'] == v_sel]['id'].values[0]
            c1, c2 = st.columns(2); s_v, t_v, p_v = c1.date_input("SOAT"), c1.date_input("Tecno"), c1.date_input("Preventivo")
