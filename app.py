...
def inicializar_db():
    ...
    # Tabla Hoja de Vida con NUEVOS CAMPOS
    ...
    # REPARACIÓN: Agregar columnas si la tabla ya existía
    nuevas_cols = [("p_contractual", "DATE"), ("p_extracontractual", "DATE"), ("p_todoriesgo", "DATE"), ("t_operaciones", "DATE")]
    for col, tipo in nuevas_cols:
        try: cur.execute(f"ALTER TABLE hoja_vida ADD COLUMN {col} {tipo}")
...
if not st.session_state.logged_in:
    ...
    if st.sidebar.button("Ingresar"):
        conn = conectar_db(); cur = conn.cursor()
        cur.execute("SELECT nombre, rol FROM usuarios WHERE usuario = %s AND clave = %s", (user_i, pass_i))
...
# --- ⚙️ USUARIOS ---
if menu == "⚙️ Usuarios" and st.session_state.user_role == "admin":
...
# --- 📊 DASHBOARD ---
    ...
    if not df_g.empty or not df_s.empty:
        ...
        res = pd.merge(df_s.groupby('Mes')['valor_viaje'].sum(), df_g.groupby('Mes')['monto'].sum(), on='Mes', how='outer').fillna(0)
        res.columns = ['Ventas', 'Gastos']; res['Utilidad'] = res['Ventas'] - res['Gastos']
        st.table(res.sort_values(by='Mes', ascending=False).style.format("${:,.0f}"))
...
# --- 📑 HOJA DE VIDA (ACTUALIZADO CON POLIZAS Y TARJETA) ---
elif menu == "📑 Hoja de Vida":
    st.title("📑 Hoja de Vida y Documentación Legal")
    conn = conectar_db(); v_data = pd.read_sql("SELECT id, placa FROM vehiculos", conn)
    
    with st.expander("📝 Registrar/Actualizar Vencimientos"):
        with st.form("h_v"):
            v_id = int(v_data[v_data['placa'] == st.selectbox("Vehículo", v_data['placa'])]['id'].values[0])
            c1, c2 = st.columns(2)
            s_v = c1.date_input("Vence SOAT"); t_v = c1.date_input("Vence Tecno"); p_v = c1.date_input("Vence Preventivo")
            pc_v = c2.date_input("Vence Póliza Contractual"); pe_v = c2.date_input("Vence Póliza Extracontractual")
            ptr_v = c2.date_input("Vence Póliza Todo Riesgo"); to_v = st.date_input("Vence Tarjeta de Operaciones")
            ...
            if st.form_submit_button("Actualizar Todo"):
                ...
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (vehiculo_id) 
                DO UPDATE SET soat_vence=EXCLUDED.soat_vence, tecno_vence=EXCLUDED.tecno_vence, prev_vence=EXCLUDED.prev_vence,
    ...
    df_h = pd.read_sql('''SELECT v.placa, h.soat_vence, h.tecno_vence, h.prev_vence, h.p_contractual, h.p_extracontractual, h.p_todoriesgo, h.t_operaciones 
    ...
    for _, row in df_h.iterrows():
        ...
        cols = st.columns(4)
        doc_map = [
            (cols[0], "SOAT", row['soat_vence']), (cols[1], "TECNO", row['tecno_vence']), (cols[2], "PREV", row['prev_vence']), (cols[3], "T. OPER", row['t_operaciones']),
            (cols[0], "CONTRACTUAL", row['p_contractual']), (cols[1], "EXTRACONT", row['p_extracontractual']), (cols[2], "TODO RIESGO", row['p_todoriesgo'])
        ]
...
# --- 💸 GASTOS ---
    ...
    if not df_mes.empty:
        st.write("**Subtotales por Concepto:**", df_mes.groupby('tipo_gasto')['monto'].sum())
